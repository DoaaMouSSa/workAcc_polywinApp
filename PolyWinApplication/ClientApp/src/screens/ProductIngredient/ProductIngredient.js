...
const equetionSchema = Yup.object({
    Width: Yup.string().required("برجاء إدخال العرض"),
        Height: Yup.string().required("برجاء إدخال الطول")
});
...
class ProductIngredient extends Component {
    ...
    getCalcProduct =(value) => {
        ...
        defaultURLProductIngredients.get(`GetCalcProduct?productId=${productId}&width=${Width}&height=${Height}`).then(res => {
            this.setState({
